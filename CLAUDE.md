...
- `src/content/design/` — design system. `tokens.ts` declares every `--bc-*` CSS custom property + per-theme override blocks; `components.ts` declares every higher-level `.bc-*` class built from those tokens; `index.ts` runs theme bootstrap. Style modules consume vars/classes — never embed raw color literals. To switch the look-and-feel, edit `tokens.ts` or add a new `[data-bc-theme="X"]` block. The popup exposes a theme dropdown that writes to `chrome.storage.local`. Paper.nu's `.dark` class is mirrored onto `<html data-bc-mode="dark">` so each theme owns its dark variant. Note: SVG-emitting JS in `paper-ctec/{chart-histogram,hours-density,modal/*,widget-chips}.ts` still hardcodes accent colors via `setAttribute('stroke', '#66023c')`; their dark counterparts in `styles/modal-charts.ts` and `styles/modal.ts` use `[stroke="#66023c"]` attribute selectors to override. Migrating these to `el.style.stroke = "var(--bc-color-accent)"` is a known follow-up.
- `src/popup/` — popup UI with toggle switches and a "clear CTEC cache" button.
- `src/content/access-gate/` — pre-launch rollout gate: HMAC last-name codes (`code.ts`) for individual whitelisting, grad-year buckets (`constants.ts`, `grad-term-fetch.ts`) for staged release, and a remote schedule (`server-client.ts`) that also carries a kill switch + broadcast banner. Schedule URL is build-time-substituted from `BC_BUCKET_SCHEDULE_URL` in `.env`; `scripts/build.mjs` also patches the URL's origin into the manifest's `host_permissions`. The matching local dev server lives in `../better-caesar-server/`.
